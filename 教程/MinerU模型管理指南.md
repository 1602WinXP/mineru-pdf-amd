...
2026 年 5 月 21 日官方发布了新 VLM 模型 `MinerU2.5-Pro-2605-1.2B`，同时 `mineru-vl-utils` 升级到 **1.0.0**（从 0.2.8 跳到 1.0.0，是一个重大的架构变更）。
...
| 只改 `enum_class.py` 里的模型名 | ❌ | 旧 `vl-utils 0.2.8` 大概率无法加载新架构的模型 |
...
|------------|--------------|-------------|--------------|
| 3.1.15 | 0.2.8 | `MinerU2.5-Pro-2604-1.2B` | `PDF-Extract-Kit-1.0` |
| 未来版本（推测） | 1.0.0+ | `MinerU2.5-Pro-2605-1.2B` (2026-05-21 发布) | 可能更新 |
